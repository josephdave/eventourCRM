...
if(isset($_REQUEST['servicio'])){
	...
	$mensaje=$control->registrarAdicional($_REQUEST['id_servicio'],$_REQUEST['idgrupo'],$_REQUEST['servicio'],$_REQUEST['proveedor'],$_REQUEST['ubicacion'],$_REQUEST['fecha'],$_REQUEST['fecha2'],$_REQUEST['costo'],$_REQUEST['aplica'],$_REQUEST['pventa'],$_REQUEST['categoria'],$_REQUEST['tipocosto'],$_REQUEST['dia'],$_REQUEST['observaciones']);
	}
	if(isset($id_prospecto) && $id_prospecto !=0){
	$mensaje=$control->registrarAdicionalProspecto($_REQUEST['id_servicio'],$_REQUEST['idgrupo'],$_REQUEST['servicio'],$_REQUEST['proveedor'],$_REQUEST['ubicacion'],$_REQUEST['fecha'],$_REQUEST['fecha2'],$_REQUEST['costo'],$_REQUEST['aplica'],$_REQUEST['pventa'],$_REQUEST['categoria'],$_REQUEST['tipocosto'],$_REQUEST['dia'],$_REQUEST['observaciones']);
	}
...
?>
							...
							?>
							  ...
							  echo strtoupper( $fi['nombre']);?>
           				      ...
           				      <tr>
           				        ...
           				        <td><input type="datetime-local" name="fecha" id="fecha" value="<?php if(isset($servicio['fecha'])){echo date("Y-m-d\TH:i:s",strtotime($servicio['fecha']));}else{echo "0000-00-00 00:00:00";} ?>"> <select name="dia" id="dia">
           				      ...
           				      <tr>
           				        ...
           				        <td>
									...
									<?php 	if(isset($id_grupo) && $id_grupo!=0){
	?>
								<input type="text" name="costo" id="costo" placeholder="" value="<?php echo $servicio['costo'] ?>">	
									<?php }else
	if(isset($id_prospecto) && $id_prospecto !=0){ ?>
									<input type="text" name="costo" id="costo" placeholder="" value="<?php echo $servicio['costo_prospecto'] ?>">	
 ...
 if(isset($id_prospecto) && $id_prospecto != 0){
	$datos_producto=$control->datosProspecto($id_prospecto);
}
								//	  $datos_producto=$control->datosProducto($id_grupo);
