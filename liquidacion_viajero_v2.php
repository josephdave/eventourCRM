					    ...
					    <thead>
					      <tr style="position: absolute">
							...
							while ($fi4 = mysql_fetch_array($resultado4, MYSQL_ASSOC)) {
								...
								if(( $fi4['costo'] != 0) && $fi4['categoria'] != 'TIQUETES'){
							?>
							...
							//echo  $first_part;
							  ...
							  <th>Costo Total PT<br></th>
							  
							   <?php 
							
							$actividades2=array();
							
							$resultado42=$control->consultaServicios($_REQUEST['grupo']);
							$totales2 = array();
							while ($fi42 = mysql_fetch_array($resultado42, MYSQL_ASSOC)) {
								//if(($fi4['categoria'] != "TIQUETES")&&($fi4['categoria'] != "PROMOCION")&&($fi4['categoria'] != "GUIAS")&&($fi4['categoria'] != "OTROS")&&($fi4['categoria'] != "COMISIONES")){
								if(( $fi42['costo'] != 0) && $fi42['categoria'] == 'TIQUETES'){
							?>
							...
							$actividades2[]=$fi42;
							$totales2[]=0;
							$pieces2 = explode(" ", $fi42['nombre']);
							$first_part2 = implode(" ", array_splice($pieces2, 0, 4));
							//echo  $first_part;
								//	var_dump($fi4);
								$proveedor2=$control->datosProveedor($fi42['proveedor_id']);
								//	var_dump($proveedor);
									
								echo "<b>".$fi42['nombre']."</b><br/>".$proveedor2['nombre']." ".$proveedor2['razonsocial']." <br/>".$proveedor2['rut'];
								?></p></th>
                              <?php
							}
							
							} ?>
							  <th>Costo Total TK<br></th>
