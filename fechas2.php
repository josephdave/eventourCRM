...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">	
	...
	white-space: nowrap;">
                          <table data-toggle="table" data-show-refresh="false" data-show-toggle="false" data-show-columns="false" data-search="false" data-pagination="false" data-sort-name="producto" data-sort-order="desc" class="table"
  ...
  <tr>
    ...
    <?php 
	$result=$control->FechasViajeGral();
...
if($fi['estado']=="ACEPTADO" && $fi['unidad_negocio']=="GRUPOS JUVENILES"){			 ?>
  <tr>
   ...
   <?php 
		...
		$start_ts = strtotime($fi['f_salida']);
  $end_ts = strtotime($fi['f_llegada']);
  $user_ts = strtotime($minf." +".$i."days");

 
  
		
		
		if(($user_ts >= $start_ts) && ($user_ts <= $end_ts)){ echo "X";}
...
if($fi['estado']=="ACEPTADO" && $fi['unidad_negocio']!="GRUPOS JUVENILES"){			 ?>
                            <tr>
                              <td bgcolor="#CCCCCC"  width="500px"><strong><?php echo strtoupper($fi['grupo']); ?> </strong></td>
                              <?php for($i=0;$i<$dias;$i++){ ?>
                              <td ><?php 
		$fech= date('Y-m-d',strtotime($minf." +".$i."days"));
