...
if($firma =='' || !isset($firma)){
	echo "HA OCURRIDO UN ERROR, cualquier duda pongase en contacto con nosotros (572) 6604000";
	exit();
}
...
//$handle =fopen($url.'programas/grupo_print.php?plan='.$cliente['id_grupo'].'&accion=print&firma='.$firma, "r");
	...
	$ch = curl_init();
curl_setopt ($ch, CURLOPT_URL, $url.'programas/grupo_print.php');///PROGRAMA FIRMADO
...
$pdf->ImageSVG('@'.$decodedImg, $x=10, $y=$pos, $w=60, $h=60, $link='', $align='', $palign='', $border=0, $fitonpage=false);
		...
		$html=" <strong>FIRMA ACEPTACION <br>
