...
if(isset($_POST["estado2"]) ){
	...
	if(($contrato['modificacion_valor'] != 0) || ($contrato['ppal'] == 3)){
		...
		if(isset($_POST["record_adicional"]) ){
	$resultado=$control->adicionarRecord($_REQUEST['idviajero'],$_REQUEST['record_adicional']);
...
$total_terrestre += $fi5['terrestre'];
		?></td>
        <td><?php echo $fi5['aerea'];?></td>
        <td><?php echo $fi5['terrestre'];?></td>
        <td><?php echo $fi5['terrestre']+$fi5['aerea'];?></td>
      ...
      <tr>
        <td>&nbsp;</td>
        <td>TOTAL PROGRAMA</td>
        <td><?php echo $total_aerea;?></td>
        <td><?php echo $total_terrestre;?></td>
        <td><?php echo $total_terrestre+$total_aerea;?></td>
...
<select name="record" id="record" onchange="getElementById('form2<?php echo  $fi['no_documento'];?>').submit()">
							...
							while ($fd = mysql_fetch_array($r, MYSQL_ASSOC)) {?>
                                        <option value="" disabled selected>Seleccione</option>
           				              <option value="<?php echo $fd['record']; ?>"   ><?php echo $fd['nombre']." ".$fd['aerolinea'];?></option>
           				              <?php } ?>
         				              </select>
       				                                            
     				                                            </form>
								  
								  <form id="form3<?php echo  $fi['no_documento'];?>" name="form3" method="post" action="producto.php" style="margin:0;">
       				                                            <input type="hidden" id="grupo" name="grupo" value="<?php echo $_REQUEST['grupo'];?>">
       				                                            <input type="hidden" id="documento" name="documento" value="<?php echo  $fi['no_documento'];?>">
 ...
 <input type="hidden" id="idviajero" name="idviajero" value="<?php echo  $fi['id'];?>">
       				Contratos Adicionales:<br>
									  <?php 
							//	var_dump($fi['record_adicional']);
								$contratos=explode(";",$fi['record_adicional']);
								foreach($contratos as $c){
									if($c!=""){
									  $contratoactual=$control->datosContratoRecord($c);

	echo $contratoactual['nombre']."<br>";}}
									  ?>
<select name="record_adicional" id="record_adicional">
