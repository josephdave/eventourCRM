...
//var_dump($_REQUEST['valor']);
	...
	$mensaje=$control->registrarPagoProveedor($_REQUEST['idproveedor'],$_REQUEST['date'],$_REQUEST['valor'],$_REQUEST['observaciones'],$_REQUEST['moneda'],$_REQUEST['usuario'],$_REQUEST['programa'],$_REQUEST['servicio']);
...
?>
	...
	//var_dump($viajero);?>
     ...
     <?php if(isset($mensaje)){?>
							...
							//	$prv=$control->datosProveedor($fi['idprov']);
								
							//	if($prv['estado']=="APROBADO"){
							?>
                                  <option value="<?php echo $fi['id'] ?>"><?php echo strtoupper($fi['nombre'])."";?></option><?php } ?>
                                </select><?php
						//	}
							 ?>
                                ...
                                <input type="date" name="date" id="date" value="<?php echo date("Y-m-j");?>" onChange="valortrm()"></td>
           				      ...
           				      <tr>
           				        <td bgcolor="#CCCCCC">Programa Relacionado</td>
							...
							while ($fi = mysql_fetch_array($res, MYSQL_ASSOC)) {?>
           				      ...
           				      <tr>
           				        <!--<td bgcolor="#CCCCCC">Servicio Relacionado</td>
           				        <td colspan="3">
    ...
    $("#programa").change(function(){
        var deptid = $(this).val();
		$("#servicio").empty();
		$("#servicio").append("<option value='0'>N/A</option>");

        $.ajax({
            url: 'api.php',
            type: 'get',
            data: {servicios:deptid},
            dataType: 'text json',
