...
<?php 
	...
	if(isset($_REQUEST['borrar'])){
		...
		$control->borrarPagoProveedor($_REQUEST['borrar']);
...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">	
					...
					<div class="panel-body">
							...
							?>
           				          <td><a href="datos_proveedor.php?id=<?php echo strtoupper($fi['id_proveedor']);?>"><?php echo strtoupper($fi['nombre']);?></a> </td>
           				          <td><?php echo strtoupper($fi['estado']);?></td>
           				          <td><?php  $producto=$control->datosProducto($fi['id_producto']);
							  echo strtoupper($producto['grupo']);?></td>
           				          <td><?php  $servicio=$control->consultaServicioID($fi['id_servicio']);
							  echo strtoupper($servicio['nombre']);?></td>
           				          <td>
									  <?php  $usuario=$control->datosUsuario($fi['usuario']);
					...
					//			  var_dump($fi['valor_FEE']);
								 ...
								 // echo $linea;
								  ?>
                                   <!-- <button type="button" data-clipboard-text="<?php echo $linea?>">Copiar</button>-->
									  <?php if($fi['estado']=="APROBADO"){ ?>
                                  <input type="checkbox" name="validado[]" value="<?php echo $fi['id']?>" id="checkbox">
									  <input type="date" name="fecha_pago_<?php echo $fi['id']?>" value="<?php echo date("Y-m-j")?>" >
									  <br>
									  Valor pagado:<br>
								    <input type="text" name="valor_pago_<?php echo $fi['id']?>" value="<?php echo number_format($fi['valor'],0,"","");?>" placeholder="valor pagado" >
									    <br>
									   TRM:<br>
								    <input type="text" name="trm_<?php echo $fi['id']?>" value="" placeholder="TRM" >
									<br> Comprobante:<br>
								    <input type="text" name="comprobante_<?php echo $fi['id']?>" value="" placeholder="Comprobante Egreso" >
									  
                                  <br>
									  <?php }else{ echo "El proveedor debe estar aprobado para registra el pago"; }?>
                                <!-- <a href="#" onClick="verCorreo('correo<?php echo $fi['id']?>')"> Correo </a>
                                  <div style="width:100%;display:none;" id="correo<?php echo $fi['id']?>"><?php echo $correo;?></div></td>-->
           				          <td><a href="pagos_proveedores_validar.php?borrar=<?php echo $fi['id']?>" onclick="return confirm('Desea Borrar?')">X</a>
                             </td>
       				            </tr>
           				        <?php } ?>
       				          </table>
           				      ...
           				      <p>
								  <input type="hidden" name="usuario_valido" id="usuario_valido" value="<?php echo $_SESSION['id'];?>">
